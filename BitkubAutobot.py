...
def rebalance():
  ...
  fix_value = 2500  # ใส่จำนวนเงินที่ต้องการrebalance.
  min_rb = 25  # Rebalanceขั้นต่ำ
  ...
  if balance_value > fix_value:
      ...
      if amount > min_rb:  # มูลค่าเพิ่มมากกว่าเท่าไหร่ถึงจะแจ้งเตือน
          ...
          # print(sell)
          bitkub.place_ask(sym='THB_KUB', amt=sell, rat=last_price, typ='limit')
      else:
  ...
  # Buy
      ...
      if amount > min_rb:  # มูลค่าลดมากกว่าเท่าไหร่ถึงจะแจ้งเตือน
          ...
          buy = int(amount)
          bitkub.place_bid(sym='THB_KUB', amt=buy, rat=last_price, typ='limit')
      else:
          print('Rebalance : Waiting')
